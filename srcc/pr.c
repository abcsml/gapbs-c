...
static ScoreT *PageRankPullGS(const Graph *g, int max_iters, double epsilon,
  ...
  if (scores == NULL || outgoing == NULL) {
    printf("Allocation failure in PageRank\n");
    exit(EXIT_FAILURE);
  }
  for (NodeID n = 0; n < num_nodes; ++n) {
    scores[n] = init_score;
    int64_t degree = GraphOutDegree(g, n);
    outgoing[n] = degree > 0 ? init_score / degree : 0;
  }
  for (int iter = 0; iter < max_iters; ++iter) {
    ...
    for (NodeID u = 0; u < num_nodes; ++u) {
      ...
      outgoing[u] = degree > 0 ? scores[u] / degree : 0;
    }
    if (logging_enabled)
...
int main(int argc, char **argv) {
  ...
  for (int iter = 0; iter < trials; ++iter) {
    ...
    ScoreT *scores = PageRankPullGS(&g, CLPageRankMaxIters(&cli),
